...
- **时间同步**：WiFi SNTP 后台校时（支持两组凭据依次尝试），每 6 小时重同步窗口；断网按"锚点 + 开机时长"推算走时；无 RTC 也能显示日期时间
...
4. 选择 [`releases/whiplash-esp32c3-v1.2.0-merged.bin`](releases/whiplash-esp32c3-v1.2.0-merged.bin)（含引导+分区表的完整镜像，从 0x0 烧写）
...
- 网络属于可选子系统：WiFi/SNTP 运行期失败只记日志并等下次重试，番茄钟本体不受影响
